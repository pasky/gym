...
try:
    with sync_playwright() as p:
        ...
        print('== upload policy: few commits')
        W = device(browser, 'W')
        W.goto(BASE + f'#/connect/pasky/gym-data/{TOK}'); sync(W)
        n0 = len(gh.commits)
        W.goto(BASE + '#/'); W.click('[data-a=pick][data-ex=lat-pulldown]'); W.wait_for_selector('#item-0')
        for j in range(3):
            W.click(f'[data-a=tick][data-i="0"][data-j="{j}"]')
        W.goto(BASE + '#/targets'); W.fill('[data-f=target][data-ex=face-pull][data-k=reps]', '14'); W.press('[data-f=target][data-ex=face-pull][data-k=reps]', 'Tab')
        W.evaluate('document.activeElement.blur()'); W.wait_for_timeout(1500)
        check(len(gh.commits) == n0, 'edits alone make no commit')
        check('•' in W.inner_text('#syncst'), 'header shows unsynced changes')
        W.reload(); W.wait_for_selector('#main'); W.wait_for_timeout(1500)
        check(len(gh.commits) == n0 + 1, 'app start with unsynced changes uploads once')
        check('•' not in W.inner_text('#syncst'), 'unsynced marker cleared')
        W.reload(); W.wait_for_selector('#main'); W.wait_for_timeout(1500)
        check(len(gh.commits) == n0 + 1, 'clean app start makes no commit')
        W.goto(BASE + f"#/s/{state(W)['active']}"); W.wait_for_selector('#item-0')
        W.click('[data-a=finish]'); W.wait_for_timeout(1500)
        check(len(gh.commits) == n0 + 2, 'finish uploads')
        W.evaluate('SY.idleMs = 400')
        W.goto(BASE + '#/ex/face-pull'); W.fill('[data-f=exnote]', 'rope, 2nd hole'); W.evaluate('document.activeElement.blur()')
        W.wait_for_timeout(2000)
